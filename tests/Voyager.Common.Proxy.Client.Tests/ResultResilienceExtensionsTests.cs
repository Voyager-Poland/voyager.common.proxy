...
public class ResultResilienceExtensionsTests
{
	...
	public void TransientErrorPolicy_ExponentialBackoff_CalculatesCorrectly()
	{
		...
		policy(1, error).GetValueOrThrow().Should().Be(1000);  // 1000 * 2^0
		policy(2, error).GetValueOrThrow().Should().Be(2000);  // 1000 * 2^1
		policy(3, error).GetValueOrThrow().Should().Be(4000);  // 1000 * 2^2
		policy(4, error).GetValueOrThrow().Should().Be(8000);  // 1000 * 2^3
	}
	...
	public void CustomRetryPolicy_LinearBackoff_CalculatesCorrectly()
	{
		...
		policy(1, error).GetValueOrThrow().Should().Be(500);
		policy(2, error).GetValueOrThrow().Should().Be(1000);
		policy(3, error).GetValueOrThrow().Should().Be(1500);
	}
